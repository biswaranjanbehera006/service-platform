...
def not_authorized(request):
    return render(request, 'admin_panel/not_authorized.html', {
...
def provider_requests(request):
    ...
    return render(request, 'admin_panel/provider_requests.html', {
...
def approve_provider(request, id):
    ...
    application = get_object_or_404(ProviderApplication, id=id)
    ...
    provider, created = Provider.objects.get_or_create(user=application.user)
    provider.services.set(application.services.all())

    messages.success(request, f"✅ {application.user.username} approved")
...
def reject_provider(request, id):
    ...
    messages.error(request, f"❌ {application.user.username} rejected")
...
def toggle_user(request, id):
    ...
    if user == request.user:
        messages.warning(request, "⚠️ You cannot block yourself.")
...
# =====================================================
# 👨‍🔧 PROVIDER MANAGEMENT (SORTED BY RATING)
# =====================================================
@login_required
def manage_providers(request):
    ...
    providers = Provider.objects.select_related('user').all().order_by('-rating')
    ...
    return render(request, 'admin_panel/providers.html', {
        'providers': providers
    })
...
def toggle_provider(request, id):
    ...
    messages.success(request, "Provider status updated ✅")
...
def delete_provider(request, id):
    ...
    messages.error(request, "Provider deleted ❌")
